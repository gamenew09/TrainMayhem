...
LINK_ENTITY_TO_CLASS(player, CTrainMayhemPlayer);
...
BEGIN_DATADESC(CTrainMayhemPlayer)
END_DATADESC()
...
CTrainMayhemPlayer::~CTrainMayhemPlayer(void)
{
...
void CTrainMayhemPlayer::PickDefaultSpawnTeam(void)
{
	if (GetTeamNumber() == 0)
	{
		...
		{
			CTeam *pCombine = g_Teams[TEAM_RED];
			CTeam *pRebels = g_Teams[TEAM_BLUE];
...
void CTrainMayhemPlayer::Spawn(void)
{
	...
	if (TrainMayhemRules()->IsIntermission())
	{
		AddFlag(FL_FROZEN);
	}
	else
...
void CTrainMayhemPlayer::SetPlayerTeamModel(void)
{
	const char *szModelName = NULL;
	szModelName = engine->GetClientConVarValue(engine->IndexOfEdict(edict()), "cl_playermodel");
...
void CTrainMayhemPlayer::SetPlayerModel(void)
{
	const char *szModelName = NULL;
...
void CTrainMayhemPlayer::SetupPlayerSoundsByModel(const char *pModelName)
{
	m_iPlayerSoundType = (int)PLAYER_SOUNDS_NORMAL;
}

void CTrainMayhemPlayer::ResetAnimation(void)
{
	if (IsAlive())
	{
		SetSequence(-1);
...
bool CTrainMayhemPlayer::Weapon_Switch(CBaseCombatWeapon *pWeapon, int viewmodelindex)
{
...
void CTrainMayhemPlayer::PreThink(void)
{
...
void CTrainMayhemPlayer::PostThink(void)
{
	BaseClass::PostThink();
...
void CTrainMayhemPlayer::PlayerDeathThink()
{
	if (!IsObserver())
	{
...
void CTrainMayhemPlayer::FireBullets(const FireBulletsInfo_t &info)
{
	...
	CWeaponTrainMayhemBase *pWeapon = dynamic_cast<CWeaponTrainMayhemBase *>(GetActiveWeapon());

	if (pWeapon)
	{
		modinfo.m_iPlayerDamage = modinfo.m_flDamage = pWeapon->GetTrainMayhemWpnData().m_iPlayerDamage;
	}
...
void CTrainMayhemPlayer::NoteWeaponFired(void)
{
...
bool CTrainMayhemPlayer::WantsLagCompensationOnEntity(const CBasePlayer *pPlayer, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits) const
{
...
Activity CTrainMayhemPlayer::TranslateTeamActivity(Activity ActToTranslate)
{
	if (m_iModelType == TEAM_RED)
...
void CTrainMayhemPlayer::SetAnimation(PLAYER_ANIM playerAnim)
{
	int animDesired;
...
//-----------------------------------------------------------------------------
bool CTrainMayhemPlayer::BumpWeapon(CBaseCombatWeapon *pWeapon)
{
...
void CTrainMayhemPlayer::ChangeTeam(int iTeam)
{
	...
	if (TrainMayhemRules()->IsTeamplay() != true && iTeam != TEAM_SPECTATOR)
	{
		//don't let them try to join combine or rebels during deathmatch.
		iTeam = TEAM_UNASSIGNED;
	}

	if (TrainMayhemRules()->IsTeamplay() == true)
	{
...
bool CTrainMayhemPlayer::HandleCommand_JoinTeam(int team)
{
...
bool CTrainMayhemPlayer::ClientCommand(const CCommand &args)
{
...
void CTrainMayhemPlayer::CheatImpulseCommands(int iImpulse)
{
	switch (iImpulse)
	{
	case 101:
...
bool CTrainMayhemPlayer::ShouldRunRateLimitedCommand(const CCommand &args)
{
...
void CTrainMayhemPlayer::CreateViewModel(int index /*=0*/)
{
...
bool CTrainMayhemPlayer::BecomeRagdollOnClient(const Vector &force)
{
	return true;
}
...
void CTrainMayhemPlayer::CreateRagdollEntity(void)
{
	if (m_hRagdoll)
	{
		UTIL_RemoveImmediate(m_hRagdoll);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CTrainMayhemPlayer::FlashlightIsOn(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTrainMayhemPlayer::FlashlightTurnOn(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CTrainMayhemPlayer::FlashlightTurnOff(void)
{
...
void CTrainMayhemPlayer::Weapon_Drop(CBaseCombatWeapon *pWeapon, const Vector *pvecTarget, const Vector *pVelocity)
{
	/*
...
void CTrainMayhemPlayer::Event_Killed(const CTakeDamageInfo &info)
{
...
int CTrainMayhemPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
...
void CTrainMayhemPlayer::DeathSound(const CTakeDamageInfo &info)
{
...
{
	CTrainMayhemPlayer *pPlayer = ToTrainMayhemPlayer(UTIL_GetCommandClient());

	int iTimeRemaining = (int)TrainMayhemRules()->GetMapRemainingTime();
...
void CTrainMayhemPlayer::Reset()
{
	ResetDeathCount();
	ResetFragCount();
}

void CTrainMayhemPlayer::CheckChatText(char *p, int bufsize)
{
	...
	//TrainMayhemRules()->CheckChatForReadySignal(this, pReadyCheck);
}

void CTrainMayhemPlayer::State_Transition(TrainMayhemPlayerState newState)
{
	State_Leave();
...
void CTrainMayhemPlayer::State_Enter(TrainMayhemPlayerState newState)
{
...
void CTrainMayhemPlayer::State_Leave()
{
...
void CTrainMayhemPlayer::State_PreThink()
{
...
CTrainMayhemPlayerStateInfo *CTrainMayhemPlayer::State_LookupInfo(TrainMayhemPlayerState state)
{
	// This table MUST match the 
	static CTrainMayhemPlayerStateInfo playerStateInfos[] =
	{
		{ STATE_ACTIVE,			"STATE_ACTIVE",			&CTrainMayhemPlayer::State_Enter_ACTIVE, NULL, &CTrainMayhemPlayer::State_PreThink_ACTIVE },
		{ STATE_OBSERVER_MODE,	"STATE_OBSERVER_MODE",	&CTrainMayhemPlayer::State_Enter_OBSERVER_MODE,	NULL, &CTrainMayhemPlayer::State_PreThink_OBSERVER_MODE }
	};
...
bool CTrainMayhemPlayer::StartObserverMode(int mode)
{
...
void CTrainMayhemPlayer::StopObserverMode()
{
	m_bEnterObserver = false;
	BaseClass::StopObserverMode();
}

void CTrainMayhemPlayer::State_Enter_OBSERVER_MODE()
{
...
void CTrainMayhemPlayer::State_PreThink_OBSERVER_MODE()
{
...
void CTrainMayhemPlayer::State_Enter_ACTIVE()
{
...
void CTrainMayhemPlayer::State_PreThink_ACTIVE()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTrainMayhemPlayer::CanHearAndReadChatFrom(CBasePlayer *pPlayer)
{
