...
class CTrainMayhemPlayerMove : public CPlayerMove
{
	DECLARE_CLASS(CTrainMayhemPlayerMove, CPlayerMove);
public:
	CTrainMayhemPlayerMove() :
...
static CTrainMayhemPlayerMove g_PlayerMove;
...
static CTrainMayhemMoveData g_TrainMayhemMoveData;
CMoveData *g_pMoveData = &g_TrainMayhemMoveData;

IPredictionSystem *IPredictionSystem::g_pPredictionSystems = NULL;

void CTrainMayhemPlayerMove::SetupMove(CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move)
{
	// Call the default SetupMove code.
	BaseClass::SetupMove(player, ucmd, pHelper, move);

	// Convert to HL2 data.
	CTrainMayhemPlayer *pHLPlayer = static_cast<CTrainMayhemPlayer*>(player);
	Assert(pHLPlayer);

	CTrainMayhemMoveData *pTrainMayhemMove = static_cast<CTrainMayhemMoveData*>(move);
	Assert(pTrainMayhemMove);

	player->m_flForwardMove = ucmd->forwardmove;
	player->m_flSideMove = ucmd->sidemove;

	pTrainMayhemMove->m_bIsSprinting = false;
...
void CTrainMayhemPlayerMove::FinishMove(CBasePlayer *player, CUserCmd *ucmd, CMoveData *move)
{
	...
	if (gpGlobals->frametime != 0)
	{
		...
		if (distance > 0)
		{
			gamestats->Event_PlayerTraveled(player, distance, pVehicle ? true : false, !pVehicle); // && static_cast< CTrainMayhemPlayer * >(player)->IsSprinting()
		}
	}
