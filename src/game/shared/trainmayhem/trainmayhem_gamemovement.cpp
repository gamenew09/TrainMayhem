...
//=============================================================================//
#include "cbase.h"
#include "trainmayhem_gamemovement.h"
#include "in_buttons.h"
#include "utlrbtree.h"
//#include "trainmayhem_shareddefs.h"
...
// Purpose: 
//-----------------------------------------------------------------------------
CTrainMayhemGameMovement::CTrainMayhemGameMovement()
{
}
...
// Output : int
//-----------------------------------------------------------------------------
int CTrainMayhemGameMovement::GetCheckInterval(IntervalType_t type)
{
...
//-----------------------------------------------------------------------------
bool CTrainMayhemGameMovement::IsForceMoveActive()
{
...
//-----------------------------------------------------------------------------
void CTrainMayhemGameMovement::SwallowUseKey()
{
	mv->m_nOldButtons |= IN_USE;
	player->m_afButtonPressed &= ~IN_USE;

	//GetTrainMayhemPlayer()->m_bPlayUseDenySound = false; // Add this value back into mayhem player.
}
...
//			*ladder - 
//-----------------------------------------------------------------------------
void CTrainMayhemGameMovement::StartForcedMove(bool mounting, float transit_speed, const Vector& goalpos, CFuncLadder *ladder)
{
	LadderMove_t* lm = GetLadderMove();
	Assert(lm);
	...
	if (ladder)
	{
		ladder->PlayerGotOn(GetTrainMayhemPlayer());
...
//-----------------------------------------------------------------------------
bool CTrainMayhemGameMovement::ContinueForcedMove()
{
	LadderMove_t* lm = GetLadderMove();
	Assert(lm);
...
// Input  : &trace - ignored
//-----------------------------------------------------------------------------
bool CTrainMayhemGameMovement::OnLadder(trace_t &trace)
{
...
//			ladderOrigin - 
//-----------------------------------------------------------------------------
void CTrainMayhemGameMovement::Findladder(float maxdist, CFuncLadder **ppLadder, Vector& ladderOrigin, const CFuncLadder *skipLadder)
{
...
void CTrainMayhemGameMovement::GetSortedDismountNodeList(const Vector &org, float radius, CFuncLadder *ladder, CUtlRBTree< NearbyDismount_t, int >& list)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CTrainMayhemGameMovement::ExitLadderViaDismountNode(CFuncLadder *ladder, bool strict, bool useAlternate)
{
...
// Input  : bOnLadder - 
//-----------------------------------------------------------------------------
void CTrainMayhemGameMovement::FullLadderMove()
{
#if !defined( CLIENT_DLL )
...
bool CTrainMayhemGameMovement::CheckLadderAutoMountEndPoint(CFuncLadder *ladder, const Vector& bestOrigin)
{
...
bool CTrainMayhemGameMovement::CheckLadderAutoMountCone(CFuncLadder *ladder, const Vector& bestOrigin, float maxAngleDelta, float maxDistToLadder)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CTrainMayhemGameMovement::LookingAtLadder(CFuncLadder *ladder)
{
	if (!ladder)
	{
		return false;
...
// Input  : &trace - 
//-----------------------------------------------------------------------------
bool CTrainMayhemGameMovement::CheckLadderAutoMount(CFuncLadder *ladder, const Vector& bestOrigin)
{
#if !defined( CLIENT_DLL )
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTrainMayhemGameMovement::LadderMove(void)
{
	...
	{
		GetTrainMayhemPlayer()->ExitLadder();
		ladder = NULL;
	}
...
bool CTrainMayhemGameMovement::CanAccelerate()
{
#ifdef HL2MP 
...
static CTrainMayhemGameMovement g_GameMovement;
