...
#include "team.h"
#include "voice_gamemgr.h"
#include "trainmayhem_gameinterface.h"
#include "weapon_trainmayhembase.h"
//#include "trainmayhem_cvars.h"

//#ifdef DEBUG	// For now let's forget about bots.
...
// Required to fix symbol error for "extern ConVar sk_plr_dmg_grenade"
...
REGISTER_GAMERULES_CLASS(CTrainMayhemRules);

BEGIN_NETWORK_TABLE_NOBASE(CTrainMayhemRules, DT_TrainMayhemRules)
...
LINK_ENTITY_TO_CLASS(trainmayhem_gamerules, CTrainMayhemGameRulesProxy);
IMPLEMENT_NETWORKCLASS_ALIASED(TrainMayhemGameRulesProxy, DT_TrainMayhemGameRulesProxy)
...
{
	CTrainMayhemRules *pRules = TrainMayhemRules();
	Assert(pRules);
	*pOut = pRules;
}

BEGIN_RECV_TABLE(CTrainMayhemGameRulesProxy, DT_TrainMayhemGameRulesProxy)
RecvPropDataTable("trainmayhem_gamerules_data", 0, 0, &REFERENCE_RECV_TABLE(DT_TrainMayhemRules), RecvProxy_TrainMayhemRules)
END_RECV_TABLE()
#else
void* SendProxy_TrainMayhemRules(const SendProp *pProp, const void *pStructBase, const void *pData, CSendProxyRecipients *pRecipients, int objectID)
{
	CTrainMayhemRules *pRules = TrainMayhemRules();
	Assert(pRules);
	return pRules;
...
BEGIN_SEND_TABLE(CTrainMayhemGameRulesProxy, DT_TrainMayhemGameRulesProxy)
SendPropDataTable("trainmayhem_gamerules_data", 0, &REFERENCE_SEND_TABLE(DT_TrainMayhemRules), SendProxy_TrainMayhemRules)
END_SEND_TABLE()
#endif
...
CTrainMayhemRules::CTrainMayhemRules()
{
#ifndef CLIENT_DLL
...
const CViewVectors* CTrainMayhemRules::GetViewVectors()const
{
	return &g_HL2MPViewVectors;
}

const HL2MPViewVectors* CTrainMayhemRules::GetHL2MPViewVectors()const
{
	return &g_HL2MPViewVectors;
}

CTrainMayhemRules::~CTrainMayhemRules(void)
{
#ifndef CLIENT_DLL
...
void CTrainMayhemRules::CreateStandardEntities(void)
{
...
//=========================================================
float CTrainMayhemRules::FlWeaponRespawnTime(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
...
bool CTrainMayhemRules::IsIntermission(void)
{
#ifndef CLIENT_DLL
...
void CTrainMayhemRules::PlayerKilled(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
#ifndef CLIENT_DLL
	if (IsIntermission())
		return;
...
void CTrainMayhemRules::Think(void)
{
	...
	if (flFragLimit) // Currently the gamerules are based upon HL2MP gamerules, we will fix this later.
	{
		if (IsTeamplay() == true)
		{
			/*
...
void CTrainMayhemRules::GoToIntermission(void)
{
#ifndef CLIENT_DLL
	if (g_fGameOver)
		return;
...
bool CTrainMayhemRules::CheckGameOver()
{
#ifndef CLIENT_DLL
	if (g_fGameOver)   // someone else quit the game already
	{
...
// to spawn again.
//=========================================================
float CTrainMayhemRules::FlWeaponTryRespawn(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
...
//=========================================================
Vector CTrainMayhemRules::VecWeaponRespawnSpot(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
	CWeaponTrainMayhemBase *pHL2Weapon = dynamic_cast< CWeaponTrainMayhemBase*>(pWeapon);
...
CWeaponTrainMayhemBase* IsManagedObjectAWeapon(CBaseEntity *pObject)
{
	return dynamic_cast< CWeaponTrainMayhemBase*>(pObject);
}
...
bool GetObjectsOriginalParameters(CBaseEntity *pObject, Vector &vOriginalOrigin, QAngle &vOriginalAngles)
{
	...
	else if (CWeaponTrainMayhemBase *pWeapon = IsManagedObjectAWeapon(pObject))
	{
...
void CTrainMayhemRules::ManageObjectRelocation(void)
{
...
//=========================================================
void CTrainMayhemRules::AddLevelDesignerPlacedObject(CBaseEntity *pEntity)
{
...
//=========================================================
void CTrainMayhemRules::RemoveLevelDesignerPlacedObject(CBaseEntity *pEntity)
{
	if (m_hRespawnableItemsAndWeapons.Find(pEntity) != -1)
	{
...
// Some game variations may choose to randomize spawn locations
//=========================================================
Vector CTrainMayhemRules::VecItemRespawnSpot(CItem *pItem)
{
...
//=========================================================
QAngle CTrainMayhemRules::VecItemRespawnAngles(CItem *pItem)
{
...
//=========================================================
float CTrainMayhemRules::FlItemRespawnTime(CItem *pItem)
{
...
//=========================================================
bool CTrainMayhemRules::CanHavePlayerItem(CBasePlayer *pPlayer, CBaseCombatWeapon *pItem)
{
	if (weaponstay.GetInt() > 0)
	{
...
//=========================================================
int CTrainMayhemRules::WeaponShouldRespawn(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
...
//-----------------------------------------------------------------------------
void CTrainMayhemRules::ClientDisconnected(edict_t *pClient)
{
#ifndef CLIENT_DLL
...
// Deathnotice. 
//=========================================================
void CTrainMayhemRules::DeathNotice(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
#ifndef CLIENT_DLL
...
void CTrainMayhemRules::ClientSettingsChanged(CBasePlayer *pPlayer)
{
#ifndef CLIENT_DLL

	CTrainMayhemPlayer *pHL2Player = ToTrainMayhemPlayer(pPlayer);
	...
	if (stricmp(szModelName, pCurrentModel))
	{
		...
		if (TrainMayhemRules()->IsTeamplay() == false)
		{
		...
		{
			if (Q_stristr(szModelName, "models/human")) // Team change.
			{
				pHL2Player->ChangeTeam(TEAM_RED);
			}
			else
			{
				pHL2Player->ChangeTeam(TEAM_BLUE);
			}
		}
	}
...
int CTrainMayhemRules::PlayerRelationship(CBaseEntity *pPlayer, CBaseEntity *pTarget)
{
#ifndef CLIENT_DLL
...
const char *CTrainMayhemRules::GetGameDescription(void)
{
	if (IsTeamplay())
...
bool CTrainMayhemRules::IsConnectedUserInfoChangeAllowed(CBasePlayer *pPlayer)
{
	return true;
}

float CTrainMayhemRules::GetMapRemainingTime()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTrainMayhemRules::Precache(void)
{
	CBaseEntity::PrecacheScriptSound("AlyxEmp.Charge");
}

bool CTrainMayhemRules::ShouldCollide(int collisionGroup0, int collisionGroup1)
{
...
bool CTrainMayhemRules::ClientCommand(CBaseEntity *pEdict, const CCommand &args)
{
#ifndef CLIENT_DLL
	...
	CTrainMayhemPlayer *pPlayer = (CTrainMayhemPlayer *)pEdict;
...
#else
/* For now let's not deal with bots.
#ifdef DEBUG
...
#endif
*/
bool CTrainMayhemRules::FShouldSwitchWeapon(CBasePlayer *pPlayer, CBaseCombatWeapon *pWeapon)
{
...
void CTrainMayhemRules::RestartGame()
{
	// bounds check
	if (mp_timelimit.GetInt() < 0)
	{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		CTrainMayhemPlayer *pPlayer = (CTrainMayhemPlayer*)UTIL_PlayerByIndex(i);
...
void CTrainMayhemRules::CleanUpMap()
{
	...
	while (pCur)
	{
		...
		*/
		...
		//else 
		if (!FindInList(s_PreserveEnts, pCur->GetClassname())) // For now we will only care about entitys that are required to be restored.
		{
			UTIL_Remove(pCur);
		}
...
void CTrainMayhemRules::CheckRestartGame(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CTrainMayhemRules::GetChatFormat(bool bTeamOnly, CBasePlayer *pPlayer)
{
